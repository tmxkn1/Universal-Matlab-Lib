function [ trans,d_avg ] = FuncICP_Main(P_mp,P_surf,tou,d_min,k_max,n_o_app,trans_init)
...
while d(end)>=d_min && k_count<=n_o_app
    ...
    while converge<2 && d(end)>d_min && k<k_max
        ...
        [ X,~,~] = FuncICP_closestPoint_m(P,P_surf);
        
        
        % The centre of gravity is found for both sets of points
        ...
        timeCost(k) = toc;
        if k/k_max>nextProgressReport;
            disp(['k count: ',num2str(k), '; deviation = ',num2str(round(d(k),6))]);
            timeEst = mean(timeCost)*(k_max-k)/60;
